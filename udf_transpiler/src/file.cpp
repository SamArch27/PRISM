...
void insert_def_and_reg(const string & defs, const string & regs, int udf_count){
    ...
    // search for the string "/* ==== Unique identifier to indicate change required: 9340jfsa034 ==== */"
    size_t search_start = 0;
    string change_start_str = "/* ==== Unique identifier to indicate change required start: 9340jfsa034 ==== */\n";
    string change_end_str = "/* ==== Unique identifier to indicate change required end: 9340jfsa034 ==== */\n";
    size_t change_start, change_end;
    try{
        while((change_start = template_str.find(change_start_str, search_start)) != string::npos){
        change_end = template_str.find(change_end_str, change_start);
        cout<<"changing: "<<template_str.substr(change_start, change_end - change_start)<<endl;
        size_t udf_start = template_str.find("udf", change_start);
        ASSERT(udf_start != string::npos, "Expect to find word udf in: "+template_str.substr(change_start, change_end - change_start));
        template_str = template_str.replace(udf_start, 4, "udf" + std::to_string(udf_count));
    ...
    template_str = template_str.replace(template_str.find("udf"), 4, "udf" + std::to_string(udf_count));
