 ...
 */
void Transpiler::parse_assignment(const string &query, string &lvalue,
                                  string &rvalue) {
...
string Transpiler::translate_query(json &query,
                                   duckdb::CodeInsertionPoint &insert,
                                   UDFType *expected_type,
                                   bool query_is_assignment = false) {
  ASSERT(query.is_string(), "Query statement should be a string.");
  string result;
  if (query_is_assignment) {
    ASSERT(expected_type == NULL,
           "In assignment, expected_type should be NULL.");
    string lvalue;
    string rvalue;
    parse_assignment(query, lvalue, rvalue);

    // QueryTranspiler query_transpiler(function_info.get(), rvalue,
    // expected_type, config, catalog); result = query_transpiler.run();
    ...
    result += code_generator.run(connection, "select " + rvalue,
                                 function_info->get_vars(), insert);
  } else {
    // QueryTranspiler query_transpiler(function_info.get(), query,
    // expected_type, config, catalog); result = query_transpiler.run();
    // duckdb::CodeInsertionPoint insert(function_info->string_function_count);
    duckdb::LogicalOperatorCodeGenerator code_generator;
    // string quer
    result = code_generator.run(connection, "select " + query.get<string>(),
                                function_info->get_vars(), insert);
  }
  // cout<<result<<endl;
...
string Transpiler::translate_expr(json &expr,
                                  duckdb::CodeInsertionPoint &insert,
                                  UDFType *expected_type,
                                  bool query_is_assignment = false) {
  if (expr.contains("query") and expr.size() == 1) {
    return translate_query(expr["query"], insert, expected_type,
                           query_is_assignment);
  } else {
...
/**
 * example: {'lineno': 5, 'varno': 3, 'expr': {'PLpgSQL_expr': {'query': 'pd2 :=
 * pd'}}
 */
string Transpiler::translate_assign_stmt(json &assign) {
  ASSERT(assign["expr"].contains("PLpgSQL_expr"),
...
string Transpiler::translate_return_stmt(json &stmt) {
  ...
  ASSERT(stmt["expr"].contains("PLpgSQL_expr"),
         "Return_stmt expression should have PLpgSQL_expr.");
  duckdb::CodeInsertionPoint insert(function_info->string_function_count);
  string ret = fmt::format(
      "{};\nreturn;\n", function_info->func_return_type.create_duckdb_value(
                            config->function["return_name"].Scalar(),
                            translate_expr(stmt["expr"]["PLpgSQL_expr"], insert,
                                           &function_info->func_return_type)));
  return insert.to_string() + ret;
}

string Transpiler::translate_cond_stmt(json &cond_stmt,
                                       duckdb::CodeInsertionPoint &insert) {
  ASSERT(cond_stmt.contains("PLpgSQL_expr"),
         "cond stmt should contain PLpgSQL_expr.");
  string tmp = "BOOL";
  UDFType cond_type(tmp);
...
string Transpiler::translate_if_stmt(json &if_stmt) {
  ...
  if (if_stmt.contains("elsif_list")) {
    for (auto &elseif_outer : if_stmt["elsif_list"]) {
      ...
      elseifs += fmt::format(
          fmt::runtime(config->control["else_if"].Scalar()),
  ...
  if (if_stmt.contains("else_body")) {
    _else = fmt::format(
        fmt::runtime(config->control["else"].Scalar()),
        fmt::arg("else_body", translate_body(if_stmt["else_body"])));
    _else += "\n";
  }
  return insert.to_string() +
         fmt::format(fmt::runtime(config->control["if_block"].Scalar()),
                     fmt::arg("condition", cond_expr),
                     fmt::arg("then_body", then_body),
                     fmt::arg("elseifs", elseifs), fmt::arg("else", _else));
}
...
string Transpiler::translate_for_stmt(json &for_stmt) {
  ...
  ASSERT(!function_info->func_vars.contains(tmp_var_name) and
             !function_info->tmp_var_substitutes.contains(tmp_var_name),
         fmt::format("temporary loop variable {} cannot be used elsewhere",
                     tmp_var_name));
  string for_var_type = "INTEGER";
  function_info->func_vars[tmp_var_name] =
      VarInfo(-1, for_var_type, udf_str, true);
  ...
  if (for_stmt.contains("step")) {
    step_size = translate_expr(for_stmt["step"]["PLpgSQL_expr"], insert,
                               &tmp_int_type, false);
  }
  // todo: uncomment when trans_expr is done
  // ASSERT(stoi(step_size) > 0, "Step size should be positive.");
  string start = translate_expr(for_stmt["lower"]["PLpgSQL_expr"], insert,
                                &tmp_int_type, false);
  string end = translate_expr(for_stmt["upper"]["PLpgSQL_expr"], insert,
                              &tmp_int_type, false);
  string output =
      insert.to_string() +
      fmt::format(
          fmt::runtime(config->control[is_reverse ? "revfor" : "for"].Scalar()),
          fmt::arg("body", translate_body(for_stmt["body"])),
          fmt::arg("start", start), fmt::arg("end", end),
          fmt::arg("name", tmp_var_name), fmt::arg("step", step_size));
...
string Transpiler::translate_while_stmt(json &while_stmt) {
  ...
  return insert.to_string() +
         fmt::format(fmt::runtime(config->control["while"].Scalar()),
                     fmt::arg("body", body), fmt::arg("condition", condition));
}
...
// todo
/**
 * Get the function arguments from the datums. Datums contain arguments (before
 * found) and local variables (after found).
 */
std::string Transpiler::get_function_vars(json &datums, string &udf_str) {
  ...
  for (size_t i = 0; i < datums.size(); i++) {
    ...
    if (var_info.type.is_unknown())
      continue; // variables with UNKNOWN types are created by for loops later
                // in the code We don't need to do anything with them here
    if (scanning_func_args) {
      ...
      string extract_data_from_value = fmt::format(
          "v{}.GetValueUnsafe<{}>()", i, var_info.type.get_cpp_type());
      vars_init += fmt::format("{} {} = {};\n", var_info.type.get_cpp_type(),
                               name, extract_data_from_value);
    } else {
      if (var.contains("default_val")) {
        duckdb::CodeInsertionPoint insert(function_info->string_function_count);
        string query_result = translate_expr(var["default_val"]["PLpgSQL_expr"],
                                             insert, &var_info.type, false);
        vars_init += insert.to_string() +
                     fmt::format("{} {} = {};\n", var_info.type.get_cpp_type(),
                                 name, query_result);
        var_info.init = true;
...
vector<string> Transpiler::translate_function(json &ast, string &udf_str) {
  ...
  cc.global_functions.push_back(fmt::format(
      ...
      fmt::arg("vars_init", vars_init), fmt::arg("action", actual_body)));
 ...
 * @return a vector of three strings: the udf file; udf register; udf
 * declaration
 */
vector<string> Transpiler::run() {
  ...
  std::regex return_pattern("RETURNS\\s+(\\w+ *(\\((\\d+, *)?\\d+\\))?)",
  ...
  std::regex name_pattern("CREATE\\s+FUNCTION\\s+(\\w+)",
                          std::regex_constants::icase);
  ...
  for (size_t i = 0; i < ast.size(); i++) {
    if (ast[i].contains("PLpgSQL_function")) {
      ...
      vector<string> tmp_ret =
          translate_function(ast[i]["PLpgSQL_function"], udf_str);
      function_regs.push_back(tmp_ret[0]);
    }
  }
  pg_query_free_plpgsql_parse_result(result);
  // Optional, this ensures all memory is freed upon program exit (useful when
  // running Valgrind) (should not be used here since this is a recurring call)
  // pg_query_exit();
  vector<string> ret;
