...
public:
    ...
    std::string bound_function_to_string(duckdb::BoundFunctionExpression &exp){
        std::string result = fmt::format(fmt::runtime("{}[{}]"), exp.ToString(), duckdb::ExpressionClassToString(exp.GetExpressionClass()));
    ...
    std::string bound_comparison_to_string(duckdb::BoundComparisonExpression &exp){
        std::string result = fmt::format(fmt::runtime("{}[{}] -> {}"), exp.ToString(), duckdb::ExpressionClassToString(exp.GetExpressionClass()), 
    ...
    void VistorOperatorHelper(duckdb::BoundFunctionExpression &exp, int indent){
        std::cout<<fmt::format(fmt::runtime("{:<{}}{}"), "", indent, bound_function_to_string(exp))<<std::endl;
        for(auto &child : exp.children){
            VisitOperatorHelper(*child, indent + 4);
        }
    }

    void VistorOperatorHelper(duckdb::BoundComparisonExpression &exp, int indent){  
        std::cout<<fmt::format(fmt::runtime("{:<{}}{}"), "", indent, bound_comparison_to_string(exp))<<std::endl;
    ...
    void VisitOperatorHelper(duckdb::Expression &exp, int indent){
        ...
        case duckdb::ExpressionClass::BOUND_COMPARISON:
            VistorOperatorHelper(exp.Cast<duckdb::BoundComparisonExpression>(), indent);
            break;
        default:
            std::cout<<fmt::format(fmt::runtime("{:<{}}{}[{}]"), "", indent, exp.ToString(), duckdb::ExpressionClassToString(exp.GetExpressionClass()))<<std::endl;
            break;
        }
    }
