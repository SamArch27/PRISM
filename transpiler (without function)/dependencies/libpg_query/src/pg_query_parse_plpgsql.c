...
static PLpgSQL_type *plpgsql_build_datatype2(Oid typeOid, int32 typmod, int location, TypeName *origtypname){ 
	PLpgSQL_type *typ; 
	typ = (PLpgSQL_type *) palloc0(sizeof(PLpgSQL_type)); 
	typ->atttypmod = typmod;
	// typ->collation = collation;
	typ->typoid = typeOid;
	char *str = palloc0(8*sizeof(char));
	snprintf(str, 8, "#%d#", location);
	// printf("%s\n", str);
	typ->typname = str;
...
static PLpgSQL_function *compile_create_function_stmt(CreateFunctionStmt* stmt)
{
	...
	foreach(lc, stmt->parameters)
	{
		...
		if (param->name != NULL)
		{
			...
			// printf("%lu\n", param->argType->);
			argdtype = plpgsql_build_datatype2(param->argType->typeOid, param->argType->typemod, param->argType->location, param->argType);
