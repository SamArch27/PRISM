...
static const char *QueryNodeTypeString[] = {"FUNCTION", "EXPRESSION",
                                            "CONSTANT", "VARIABLE"};
...
public:
   ...
   * one to one mapping
   */
  vector<vector<UDFType>> input_type;
  vector<UDFType> return_type;
  HeaderFileFunctionInfo(){};
  HeaderFileFunctionInfo(string cpp_name, bool templated, bool default_null,
                         bool if_switch, bool if_string,
                         vector<vector<UDFType>> input_type,
                         vector<UDFType> return_type)
      : cpp_name(cpp_name), templated(templated), default_null(default_null),
        if_switch(if_switch), if_string(if_string), input_type(input_type),
...
public:
  ...
  UDFType type;
  ConstantInfo(){};
  ConstantInfo(string value, UDFType type) : value(value), type(type){};
};

/**
 * the binded info for each QueryNode
 * this class does not itself contain the actual info except for constants, but
 * a pointer to the actual info for example, function and expression info are
 * stored in the catalog variable info are stored in the function_info only
 * constant info are stored here
 */
...
/**
 * Essentially a table containing the QueryNode name to QueryNodeBoundInfo
 * mapping owns all the objects of QueryNodeBoundInfo
 */
class Catalog {
...
public:
  Catalog() {
    ...
    for (auto function : header_file) {
      ASSERT(function.first.IsScalar() && function.second.IsSequence(),
             fmt::format("header file around is wrong around {}",
                         function.first.Scalar()));
      ...
      ASSERT(function_info[5].IsMap() && !function_info[5]["options"].IsNull(),
             "options should be a map");
      ASSERT(function_info[5]["options"].IsSequence(),
             "input and return options should be a sequence");
      vector<vector<UDFType>> input_type;
      vector<UDFType> return_type;
      for (auto ret_input_pair_tmp : function_info[5]["options"]) {
        ASSERT(ret_input_pair_tmp.size() == 1,
               "input and return pair should be a map");
        // this loop should have only one iteration due to a wired behavior of
        // yaml-cpp that a map with only one pair will be parsed as a sequence
        // of maps with one pair
        for (auto ret_input_pair : ret_input_pair_tmp) {
          // cout<<ret_input_pair.first.Scalar()<<endl;
          ASSERT(ret_input_pair.first.IsScalar() &&
                     ret_input_pair.second.IsSequence(),
                 "key should be string and value should be a sequence");
          // cout<<ret_input_pair.first.Scalar()<<endl;
          return_type.emplace_back(ret_input_pair.first.Scalar());
          vector<UDFType> input_type_vec;
      ...
      table.emplace(function_name,
                    HeaderFileFunctionInfo(cpp_name, templated, default_null,
                                           if_switch, if_string, input_type,
                                           return_type));
    }
  }
  static void Print(Catalog catalog) {
    VariadicTable<string, string, string, string, string, string, string,
                  string>
        vt({"SQL Name", "C++ Name", "Templated", "Default NULL", "Switch",
            "String Op", "Inputs", "Return"},
           10);
    for (auto &pair : catalog.table) {
      ...
      for (int i = 0; i < return_types.size(); i++) {
        ...
        if (first) {
          vt.addRow(pair.first, pair.second.cpp_name,
                    pair.second.templated ? "true" : "false",
                    pair.second.default_null ? "true" : "false",
                    pair.second.if_switch ? "true" : "false",
                    pair.second.if_string ? "true" : "false", input_type_str,
                    return_type_str);
          first = false;
        } else
...
public:
  ...
  static void Print(const QueryNode &node, int indent = 0) {
    ...
    //     cout<<fmt::format(fmt::runtime("{:<{}}{}[{}]->{}"), "", indent,
    //     node.bound_info.constant_info->value,
    //     getQueryNodeTypeString(node.type), node.bound ? "bound" : "")<<endl;
    // else if(node.type == VARIABLE)
    //     cout<<fmt::format(fmt::runtime("{:<{}}{}[{}]->{}"), "", indent,
    //     node.name, getQueryNodeTypeString(node.type), node.bound ? "bound" :
    //     "")<<endl;
    ...
    cout << fmt::format(fmt::runtime("{:<{}}{}[{}]->{}"), "", indent, node.name,
                        getQueryNodeTypeString(node.type),
                        node.bound ? "bound" : "")
         << endl;
...
public:
  ...
  static void Print(const QueryAST &ast) { QueryNode::Print(ast.root); }
...
public:
   ...
   */
  QueryAST(json &ast) : root(resolve_node(ast)) {
    // ASSERT(ast.is_object() && ast.size() == 1, "The root ast should be a
    // map.");
  }
};
 ...
 * it controls the whole lifecycle of QueryAST including construction, binding
 * and transpiling
 */
class QueryTranspiler {
private:
  ...
  // public:
public:
  QueryTranspiler(FunctionInfo *function_info, string query_str,
                  UDF_Type *expected_type, YAMLConfig *config, Catalog *catalog)
      : function_info(function_info), query_str(query_str),
        expected_type(expected_type), config(config), catalog(catalog) {
    // cout<<query_str<<endl;
  }
