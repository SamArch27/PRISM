...
// Scalar function
//===--------------------------------------------------------------------===//
inline void hello_fun(const Value &v1, bool v1_null, Value &result,
                      bool &result_null) {
  if (v1_null) {
    result_null = true;
    return;
...
inline void HelloFunction(DataChunk &args, ExpressionState &state,
                          Vector &result) {
  ...
  for (int base_idx = 0; base_idx < count; base_idx++) {
    ...
    hello_fun(vector.GetValue(index), !data.validity.RowIsValid(index),
              temp_result, temp_result_null);
    if (temp_result_null) {
      FlatVector::SetNull(result, base_idx, true);
    } else {
      result.result.SetValue(index, temp_result);
    }
  }
...
DUCKDB_EXTENSION_API void sam_extension_init(duckdb::DatabaseInstance &db) {
  ...
  // Function add point
  ScalarFunction hello_func("hello", {LogicalTypeId::VARCHAR},
                            LogicalTypeId::INTEGER, HelloFunction);
